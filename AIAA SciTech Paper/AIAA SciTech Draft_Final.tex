...
\end{equation}
%
It should be noted that in order to calculate the Kalman gain, as seen in Eq.~\ref{eq:K}, the quantity $\textbf{H}_i\textbf{P}\textbf{H}_i^T+\textbf{R}_i$ must be invertible.  In this particular case, the null space of the measurement covariance, $\textbf{R}_{i}$ is the same as the null space of $\textbf{H}_i\textbf{P}\textbf{H}_i^T$ assuming $\textbf{R}_i$ is determined to first order using the QUEST covariance matrix, as seen in Eq.~\ref{eq:meascovR}, as is common practice with unit vector measurements \cite{Shuster:1981}.  Eq.~\ref{eq:Knew} gives the final equation used to determine the optimal Kalman gain.
%
